...
function rekapComputeWinLose(entry, resultDigits){
  if(!resultDigits) return null;

  const hasilNum = String(entry.hasil || '').replace(/[^0-9]/g, '');

  // ── 1. Gen1 (Formula X): SEMUA posisi A,C,K,E harus hijau (4/4) ──
  if(!entry.fxLabels || !entry.fxLabels.length) return null; // tidak bisa ditentukan
  ...
  // ── 2. Gen2a / Gen2b / Gen2c: lose jika 4/4 slot merah semua ──
  const gl = entry.gen2Lines || {};
  if(entry.gen2Active && !gl.live){
    const TWIN_MURNI_LABELS_4D_LOCAL = {
      '0-1':'AABC','0-2':'ABAC','0-3':'ABCA','1-2':'BAAC','1-3':'BACA','2-3':'BCAA'
    };
    ['A','B','C'].forEach(slot => {
      // sudah dihitung sebelumnya — pakai rekapColorGen2Line logika: merah jika digit
      // pada posisi label cocok digit hasil pada posisi yang sama.
      // Di sini kita hitung ulang jumlah slot yang merah dari lineStr gen2.
    });
    // Hitung merah per slot Gen2: format "A:digits, C:digits, K:digits, E:digits"
    const countGen2Red = (lineStr) => {
      if(!lineStr) return 0;
      let red = 0;
      lineStr.split(', ').forEach(tok => {
        const idx = tok.indexOf(':');
        if(idx === -1) return;
        const label = tok.slice(0, idx);
        const pool  = tok.slice(idx + 1);
        const target = resultDigits[label];
        if(target && pool.includes(target)) red++;
      });
      return red;
    };
    if(gl.A !== null && countGen2Red(gl.A) >= 4) return 'lose';
    if(gl.B !== null && countGen2Red(gl.B) >= 4) return 'lose';
    if(gl.C !== null && countGen2Red(gl.C) >= 4) return 'lose';
  }

  // ── 3. Filter AI (AC, CK, KE): setiap posisi yang diisi harus ada hijau ──
  const acHit = new Set([resultDigits.A, resultDigits.C].filter(Boolean));
  const ckHit = new Set([resultDigits.C, resultDigits.K].filter(Boolean));
  const keHit = new Set([resultDigits.K, resultDigits.E].filter(Boolean));
  const hasHit = (pool, hitSet) => pool && pool.split('').some(ch => hitSet.has(ch));
  if(entry.aiAC && !hasHit(entry.aiAC, acHit)) return 'lose';
  if(entry.aiCK && !hasHit(entry.aiCK, ckHit)) return 'lose';
  if(entry.aiKE && !hasHit(entry.aiKE, keHit)) return 'lose';

  // ── 4. Selisih & Jumlah: masing-masing harus ada hijau ──
  const d4 = resultDigits.A + resultDigits.C + resultDigits.K + resultDigits.E;
  const { jumlahList, selisihList } = jumlahSelisihList(d4);
  const jumlahSet  = new Set(jumlahList.map(String));
  const selisihSet = new Set(selisihList.map(String));
  const filterHasHit = (raw, hitSet) => {
    if(!raw) return true; // tidak diisi = tidak dicek
    const items = raw.includes(',') ? raw.split(',') : raw.split('');
    return items.some(v => hitSet.has(v.trim()));
  };
  if(entry.jumlah  && !filterHasHit(entry.jumlah,  jumlahSet))  return 'lose';
  if(entry.selisih && !filterHasHit(entry.selisih, selisihSet)) return 'lose';

  // ── 5. Shio: harus ada minimal 1 shio hijau ──
  if(entry.shio && entry.shio.length){
    const SHIO_TABLE_WL = {
      1:['01','13','25','37','49','61','73','85','97'],
      2:['02','14','26','38','50','62','74','86','98'],
      3:['03','15','27','39','51','63','75','87','99'],
    ...
    const shioHitSet = new Set();
    [resultDigits.A+resultDigits.C, resultDigits.C+resultDigits.K, resultDigits.K+resultDigits.E]
      .forEach(pair => {
        for(const [n, list] of Object.entries(SHIO_TABLE_WL)){
          if(list.includes(pair)){ shioHitSet.add(String(n)); break; }
        }
      });
    const adaShioHijau = entry.shio.some(s => shioHitSet.has(String(s).trim()));
    if(!adaShioHijau) return 'lose';
  }

  // ── 6. Twin: jika ada merah maka lose ──
  if(entry.twin && entry.twin.length){
    const hasilAdaKembar = new Set(hasilNum.split('')).size < hasilNum.length;
    const hasilAda34 = (() => {
      const cnt = {};
    ...
    const hasilAdaDiRiwayat = Array.isArray(lastHistoryNumbers) && lastHistoryNumbers.includes(hasilNum);
    const twinMurniLabels = entry.twinMurniLabels || [];
    const TWIN_LABEL_TO_PAIR_WL = {'AABC':[0,1],'ABAC':[0,2],'ABCA':[0,3],'BAAC':[1,2],'BACA':[1,3],'BCAA':[2,3]};
    const hasilCocokTwinMurni = twinMurniLabels.some(label => {
      const pair = TWIN_LABEL_TO_PAIR_WL[label];
      if(!pair) return false;
      const [i,j] = pair;
      const d = hasilNum.split('');
      if(d[i] !== d[j]) return false;
    ...
    for(const t of entry.twin){
      if(t === 'hapus 3-4'              && hasilAda34)          return 'lose';
      if(t === 'hapus twin'             && (twinMurniLabels.length ? hasilCocokTwinMurni : hasilAdaKembar)) return 'lose';
      if(t === 'angka yang sudah keluar'&& hasilAdaDiRiwayat)   return 'lose';
    }
    // pola twin murni spesifik
    if(hasilCocokTwinMurni) return 'lose';
  }
