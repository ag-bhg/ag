...
setupSectionToggle('fgCardToggle', 'fgCardWrap', 'fgCardToggleIcon');
...
function analyze(){
  ...
  // Formula Gelut punya cache data sendiri (fgHistoryNumbers/fgPosLabels, mode "Data Default"
  // ikut baca #dataInput) yang SEBELUMNYA cuma di-refresh lewat radio Sumber-nya sendiri atau
  // tombol Auto%/StreakX — jadi kalau cuma ganti Periode/Pasaran di sini, datanya jadi basi
  // (masih nunjuk ke Periode lama) sampai user klak-klik manual. Disamakan sekarang: tiap kali
  // analyze() jalan (= tiap ganti Periode/Pasaran), Formula Gelut ikut refresh cache-nya, dan
  // tabel hasilnya di-reset (bukan dihitung ulang otomatis — biar tetap ringan, user tinggal
  // klik Auto%/StreakX lagi kalau mau lihat hasil barunya) supaya tidak nampilin hasil lama
  // yang sudah tidak sesuai data baru.
  if(typeof fgRefreshHistoryIfTerbaru === 'function'){
    fgRefreshHistoryIfTerbaru();
    if(typeof FG_RESULT !== 'undefined') FG_RESULT = null;
    if(typeof FG_SELECTED_KEY !== 'undefined') FG_SELECTED_KEY = null;
    if(typeof fgRenderAll === 'function') fgRenderAll();
  }
