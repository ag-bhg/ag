...
const B = AiBrain;
const RM = typeof AiRiskManager !== 'undefined' ? AiRiskManager : null; // Risk Manager blueprint "Sekring" (opsional: kalau file belum dimuat, fitur lama tetap jalan tanpanya)
...
const A = {
  ...
  cfEntries: {},          // Tahap 1 shadow-track: key(nama pasaran) -> {pending, n, hits, log} tebakan Cloudflare AI
  streakZones: null,      // Zona Aman Streak (LAMA): hasil "pelajari zona" (pool semua pasaran termuat), null = belum pernah belajar
  guard: {},              // Risk Manager (blueprint Sekring) — nama pasaran -> guard {mode, zigIdx, out, ...} tersimpan
  guardAudit: {}          // nama pasaran -> teks audit TERAKHIR yang sudah dicetak (hindari cetak ulang isi sama)
};
...
async function load(){
  ...
  try{
    ...
    A.cfEntries = mem.cf || {};
    A.streakZones = (mem.zone && mem.zone.GLOBAL) || null;
    Object.keys(mem.guard || {}).forEach(key => {
      const g = mem.guard[key]; if(g && g.name) A.guard[g.name] = g.guard || RM && RM.newGuard();
    });
...
// ---------- Backtest walk-forward Cloudflare AI (pakai data historis yang SUDAH ADA — tidak perlu nunggu draw baru) ----------
// Latih dari draw terlama sampai batas, lalu tebak draw demi draw satu-satu MAJU (hanya memakai data
// sebelumnya, tidak bocor) — tiap giliran diberi tahu hasil tebakan sebelumnya (benar/salah) sebagai
// feedback, supaya kalau memang ada pola, Cloudflare AI punya kesempatan menyesuaikan. 1 draw = 1 kali
// panggilan API (bukan disatukan), sesuai maksud "belajar sungguhan" — bukan tebak buta sekaligus.
const BT_TRAIN_MIN = 30;
function btRowsText(C, upTo){ return C.slice(0, upTo).map(r => r.join('')).join('\n'); }
function btContext(name, L, C, upTo, feedback){
  const lab = B.posLabels(L);
  let s = 'Pasaran ' + name + '. DATA HISTORIS (kronologis lama→baru, tiap baris = 1 draw, digit berurutan posisi ' + lab.join(',') + '):\n' + btRowsText(C, upTo);
  if(feedback && feedback.length) s += '\n\nRIWAYAT TEBAKAN ANDA SESI INI (draw-draw sebelumnya, evaluasi sendiri & sesuaikan strategi kalau perlu):\n' + feedback.slice(-15).join('\n');
  return s;
}
async function runCfBacktest(reqTestN){
  const ms = markets(), cur = pickDefaultCur(ms);
  if(!cur || !ms[cur]) return say('Belum ada pasaran aktif.');
  const C = ms[cur].C, L = ms[cur].L, labels = B.posLabels(L);
  let testN = Math.min(reqTestN || 60, Math.max(0, C.length - BT_TRAIN_MIN));
  if(testN < 5) return say('Data ' + cur + ' cuma ' + C.length + ' draw — minimal butuh ' + (BT_TRAIN_MIN + 5) + ' draw untuk backtest (latih ' + BT_TRAIN_MIN + ' + uji 5). Berhenti, tidak dipaksakan.');
  if(testN < (reqTestN || 60)) say('⚠️ Data cuma cukup untuk uji ' + testN + ' draw terakhir (bukan ' + (reqTestN || 60) + '), karena minimal ' + BT_TRAIN_MIN + ' draw disisakan untuk latihan.');
  const train0 = C.length - testN;
  setBusy(true);
  const hits = {}; labels.forEach(l => { hits[l] = 0; });
  const feedback = [];
  say('🔬 Mulai backtest Cloudflare AI ' + cur + ': latih ' + train0 + ' draw awal, uji ' + testN + ' draw satu-satu dengan feedback (' + testN + ' kali panggilan API). Mohon tunggu…');
  for(let i = 0; i < testN; i++){
    const t = train0 + i;
    setStatus('Backtest Cloudflare AI ' + cur + ': draw ' + (i + 1) + '/' + testN + '…'); renderAll();
    let j;
    try{
      const resp = await fetch('/api/ai-guess', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ labels, out: A.out, context: btContext(cur, L, C, t, feedback) })
      });
      j = await resp.json();
    }catch(e){
      setBusy(false); renderAll();
      return say('⚠️ Backtest berhenti di draw ' + (i + 1) + '/' + testN + ': gagal menghubungi Cloudflare AI (' + (e && e.message ? e.message : e) + '). Hasil sebagian TIDAK dipakai/disimpan.');
    }
    if(!j || !j.ok){
      setBusy(false); renderAll();
      return say('⚠️ Backtest berhenti di draw ' + (i + 1) + '/' + testN + ': ' + (j && j.error ? j.error : 'jawaban tidak valid') + '. Hasil sebagian TIDAK dipakai/disimpan — bukan dipaksa lanjut pakai data rusak.');
    }
    const row = C[t], detail = [];
    labels.forEach((lab, p) => {
      const guess = j.guess[lab] || [], hit = guess.indexOf(row[p]) >= 0;
      if(hit) hits[lab]++;
      detail.push(lab + '=' + row[p] + (hit ? ' HIT' : ' MISS') + '(tebak:' + guess.join(',') + ')');
    });
    feedback.push('Draw ke-' + (t + 1) + ': ' + detail.join(' · '));
  }
  setBusy(false);
  // Uji jujur vs acak murni (peluang kena = OUT/10 per posisi tiap tebakan) — z-test binomial, sama semangat dengan "uji" pakar lain
  const p0 = A.out / 10, n = testN;
  const zOf = h => (h - n * p0) / Math.sqrt(n * p0 * (1 - p0));
  const lines = labels.map(lab => {
    const h = hits[lab], z = zOf(h), pv = B.pOneSided(z);
    return lab + ': kena ' + (h / n * 100).toFixed(1) + '% (' + h + '/' + n + ', acak ' + (p0 * 100).toFixed(0) + '%), z=' + z.toFixed(2) + ' (p=' + pv.toFixed(3) + ')';
  });
  const totalH = labels.reduce((s, l) => s + hits[l], 0), totalN = n * labels.length;
  const zAll = (totalH - totalN * p0) / Math.sqrt(totalN * p0 * (1 - p0));
  const sig = labels.filter(lab => zOf(hits[lab]) > 1.645).length;
  const verdict = zAll > 1.96 ? '🟢 Ada indikasi pola nyata (z gabungan ' + zAll.toFixed(2) + ', di atas ambang 1,96).' :
    (zAll > 1.645 ? '🟡 Indikasi tipis (z gabungan ' + zAll.toFixed(2) + '), belum cukup kuat untuk dipercaya.' :
    '⚪ Setara acak (z gabungan ' + zAll.toFixed(2) + ') — Cloudflare AI TIDAK menemukan pola nyata di data ini, hasil ini cuma tebakan.');
  say('HASIL BACKTEST Cloudflare AI ' + cur + ' (latih ' + train0 + ' draw, uji ' + testN + ' draw, ' + testN + ' kali usaha dgn feedback tiap giliran):\n' + lines.join('\n') +
    '\nGabungan semua posisi: kena ' + (totalH / totalN * 100).toFixed(1) + '% dari ' + totalN + ' tebakan-digit (acak ' + (p0 * 100).toFixed(0) + '%), z=' + zAll.toFixed(2) + '.\n' +
    sig + '/' + labels.length + ' posisi lolos z>1,64 (indikasi tipis, belum tentu nyata — bandingkan sama "uji semua" pakar biasa).\n' + verdict);
  renderAll();
}

// ---------- Zona Aman Streak — lapis TAMBAHAN, TIDAK mengganti prediksi mixture yang sudah ada ----------
...
function learnZones(filterText){
  ...
  A.streakZones = zones;
  if(typeof AiMemory !== 'undefined') AiMemory.saveZoneEntry('GLOBAL', zones);
  const totalEv = lab => (zones.perLabel[lab] || []).reduce((s, r) => s + r.n, 0);
  const labelsAll = Object.keys(zones.perLabel);
  const summary = labelsAll.map(lab => {
    const safe = (zones.perLabel[lab] || []).filter(r => r.safe);
...
function showExecution(){
  const ms = markets(), cur = pickDefaultCur(ms);
  if(!cur || !ms[cur] || !A.states[cur]) return say('Belum ada pasaran aktif / belum pernah dipelajari.');
  if(!A.streakZones) return say('Zona Aman Streak belum pernah dipelajari. Ketik "pelajari zona" dulu.');
  const st = A.states[cur], exec = B.pickExecution(st, A.streakZones, A.out);
  if(typeof AiMemory !== 'undefined') AiMemory.saveZoneEntry(cur, { name: cur, exec, learnedAt: A.streakZones.at, updatedAt: Date.now() });
  const lines = Object.keys(exec.perPos).map(lab => {
    const r = exec.perPos[lab];
    return lab + ': streak sekarang ' + r.streak + (r.safe ? ' ✅ AMAN (z=' + (r.z != null ? r.z.toFixed(2) : '-') + ', n=' + r.n + ')' : ' ⚪ belum masuk zona aman' + (r.n ? ' (z=' + (r.z != null ? r.z.toFixed(2) : '-') + ', n=' + r.n + ')' : ' (belum ada data streak segini)'));
  });
  const modeText = exec.mode === 'AMAN_PER_POSISI' ? '🟢 SEMUA posisi masuk zona aman — metode/pola yang sedang jalan boleh dieksekusi.' :
    (exec.mode === 'AMAN_GABUNGAN' ? '🟡 Tidak semua posisi aman sendiri-sendiri, tapi GABUNGANNYA (streak≈' + exec.streakApprox + ', z=' + (exec.combinedZ != null ? exec.combinedZ.toFixed(2) : '-') + ') masuk zona aman.' :
    '🔴 ZIGZAG — belum ada yang masuk zona aman (per-posisi maupun gabungan). Jangan terpaku 1 pola, sebar ke beberapa kandidat terbaik.');
  say('REKOMENDASI EKSEKUSI ' + cur + ' (lapis tambahan, prediksi utama di atas TIDAK berubah):\n' + lines.join('\n') + '\n' + modeText);
}

// ---------- Risk Manager (blueprint "Sekring") — MENGGANTIKAN Zona Aman Streak di atas sbg lapis keputusan eksekusi ----------
// Beda dari Zona Aman Streak lama (pool lintas-pasaran, perlu "pelajari zona" manual dulu), guard ini
// murni per-pasaran: target & toleransi diukur langsung dari riwayat pasaran itu sendiri, jadi bisa
// dihitung ulang tiap ada data baru TANPA perintah manual — cocok utk poin 4 (silent background).
function runGuard(name, opts){
  opts = opts || {};
  if(!RM || !A.states[name]) return null;
  const st = A.states[name];
  const outNow = (A.guard[name] && A.guard[name].out) || A.out || RM.OUT_DEFAULT;
  const res = RM.evaluate(name, st, outNow, A.guard[name], opts.pr || (name === A.cur ? A.lastPred : null));
  A.guard[name] = res.guard;
  if(typeof AiMemory !== 'undefined') AiMemory.saveGuardEntry(name, { name, guard: res.guard, audit: RM.buildAudit(res), updatedAt: Date.now() });
  return res;
}
// Dipanggil untuk pasaran AKTIF saja: cetak audit ke chat HANYA kalau ada perubahan status nyata
// (mode pindah, breaker trip, target tercapai, atau OUT bergeser) — bukan tiap putaran, supaya tidak spam.
function auditActiveIfChanged(name){
  const res = runGuard(name);
  if(!res) return null;
  const notable = res.switched || res.tripped || res.reached || res.outChanged;
  if(notable && A.guardAudit[name] !== res.guard.lastSignature){
    A.guardAudit[name] = res.guard.lastSignature;
    say(RM.buildAudit(res));
  }
  return res;
}
// Sapuan LATAR BELAKANG (poin 4 blueprint): evaluasi SEMUA pasaran yang sudah dimuat (bukan cuma
// yang sedang dibuka di layar), simpan diam-diam ke cloud — TIDAK memaksa render/pindah tampilan,
// dan TIDAK mencetak apa pun ke chat kecuali untuk pasaran aktif (ditangani terpisah lewat syncData).
function guardSweepBackground(){
  if(!RM) return;
  Object.keys(A.states).forEach(name => { if(name !== A.cur) runGuard(name); });
}
// Perintah manual "audit" / "sekring": tampilkan status guard pasaran aktif kapan saja, terlepas
// apakah ada perubahan atau tidak (beda dari auditActiveIfChanged yang hanya bicara kalau berubah).
function showGuard(){
  const ms = markets(), cur = pickDefaultCur(ms);
  if(!RM) return say('Modul Risk Manager (aiRiskManager.js) belum dimuat.');
  if(!cur || !A.states[cur]) return say('Belum ada pasaran aktif / belum pernah dipelajari.');
  const res = runGuard(cur);
  say(RM.buildAudit(res));
}
// Detail Tangga Prioritas Formula pasaran aktif (tie-breaker & tampilan saja, lihat aiRiskManager.js).
function showLadder(){
  const ms = markets(), cur = pickDefaultCur(ms);
  if(!RM) return say('Modul Risk Manager (aiRiskManager.js) belum dimuat.');
  if(!cur || !A.states[cur]) return say('Belum ada pasaran aktif / belum pernah dipelajari.');
  const res = runGuard(cur);
  if(!res.ladder) return say('Tangga belum bisa dihitung untuk ' + cur + ' (data/registry formula belum cukup).');
  say(RM.buildLadderReport(cur, res.ladder));
}

// ---------- Tahap 1: shadow-track Cloudflare AI (belum ikut campuran bobot pakar aiBrain) ----------
function cfKey(name){ return (typeof AiMemory !== 'undefined' && AiMemory.encodeKey) ? AiMemory.encodeKey(name) : name; }
function getCfEntry(name){ return A.cfEntries[cfKey(name)] || null; }
function setCfEntry(name, entry){
  A.cfEntries[cfKey(name)] = entry;
  if(typeof AiMemory !== 'undefined') AiMemory.saveCfEntry(name, entry);
}
// Minta tebakan N digit (=A.out) per posisi dari Cloudflare AI untuk draw BERIKUTNYA pasaran aktif,
// simpan sebagai "pending". Kalau jawabannya tidak valid, BERHENTI & beri tahu user — tidak diam-diam dipakai.
async function askCfGuess(){
  const ms = markets(), cur = pickDefaultCur(ms);
  if(!cur || !ms[cur]) return say('Belum ada pasaran aktif.');
  const labels = B.posLabels(ms[cur].L);
  setBusy(true); setStatus('Meminta tebakan Cloudflare AI…');
  try{
    const resp = await fetch('/api/ai-guess', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ labels, out: A.out, context: contextBlock() })
    });
    const j = await resp.json();
    if(!j || !j.ok){ say('⚠️ Cloudflare AI gagal menebak: ' + (j && j.error ? j.error : 'tidak diketahui') + '. Tidak disimpan.'); return; }
    const prev = getCfEntry(cur) || {};
    const entry = { name: cur, pending: { t: ms[cur].C.length, digits: j.guess, createdAt: Date.now() },
      n: prev.n || 0, hits: prev.hits || {}, log: prev.log || [], updatedAt: Date.now() };
    setCfEntry(cur, entry);
    say('Tebakan Cloudflare AI untuk ' + cur + ' disimpan (buat draw ke-' + (entry.pending.t + 1) + '):\n' +
      labels.map(l => l + ': ' + ((j.guess[l] || []).join(' '))).join('\n') +
      '\nAkan dicek otomatis begitu hasil barunya masuk. Ini shadow-track — BELUM ikut campuran prediksi Ai.');
  }catch(e){
    say('⚠️ Gagal menghubungi Cloudflare AI: ' + (e && e.message ? e.message : e));
  }finally{
    setBusy(false); renderAll();
  }
}
// Dipanggil otomatis dari syncData() tiap ada data baru — bandingkan tebakan pending (kalau ada)
// dengan hasil real yang baru masuk, catat kena/meleset. Tidak mengubah bobot pakar aiBrain sama sekali.
function checkCfGuess(name, ms){
  const entry = getCfEntry(name); if(!entry || !entry.pending) return;
  const C = ms[name] && ms[name].C; if(!C || C.length <= entry.pending.t) return;
  const row = C[entry.pending.t], labels = B.posLabels(ms[name].L);
  const hits = Object.assign({}, entry.hits), detail = [];
  labels.forEach((lab, i) => {
    const guess = (entry.pending.digits && entry.pending.digits[lab]) || [];
    const hit = guess.indexOf(row[i]) >= 0;
    hits[lab] = (hits[lab] || 0) + (hit ? 1 : 0);
    detail.push(lab + (hit ? '✅' : '❌'));
  });
  const log = (entry.log || []).concat([{ t: Date.now(), draw: entry.pending.t, detail: detail.join(' ') }]).slice(-20);
  setCfEntry(name, { name, pending: null, n: (entry.n || 0) + 1, hits, log, updatedAt: Date.now() });
  say('📩 Hasil tebakan Cloudflare AI ' + name + ': ' + detail.join(' '));
}
...
function predictCur(){
  ...
  A.lastPred = pr; A.lastNew = r.newRows; A.lastSt = r.st;
  try{
...
// ---------- Perintah ----------
const FAMS = { nrl: ['NRL'], ml: ['ML'], mb: ['MB'], idx: ['IDX'], bhg: ['BHG'], pk: ['PK'], rdm: ['RDM'], sendiri: ['OWN'], own: ['OWN'],
  kombinasi: ['COMBO'], combo: ['COMBO'], formula: ['NRL', 'ML', 'MB', 'IDX', 'BHG', 'PK', 'RDM'] };
...
async function handle(rawText){
  ...
  if((m = t.match(/^hanya\s+(\w+)/))){
    const f = FAMS[m[1]]; if(!f) return say('Kelompok pakar: ' + Object.keys(FAMS).join(', '));
    A.famOff = {}; ['NRL', 'ML', 'MB', 'IDX', 'BHG', 'PK', 'RDM', 'OWN', 'COMBO'].forEach(x => { if(f.indexOf(x) < 0) A.famOff[x] = true; });
    refreshPredict(true); return say('Siap. Ai hanya memakai ' + m[1].toUpperCase() + '.');
  }
  if(/^pakar/.test(t)){
    const off = Object.keys(A.famOff).filter(k => A.famOff[k]);
    return say('Kelompok pakar: NRL, ML, MB, IDX, BHG, PK, RDM (formula Formula X), SENDIRI (frekuensi, jarak, transisi, pengulangan), KOMBINASI (hasil lab). Dimatikan: ' + (off.length ? off.join(', ') : 'tidak ada') + '.');
  }
  ...
  if(/^(audit|sekring|breaker)\b/.test(t)) return showGuard();
  if(/^(tangga|prioritas\s*formula|ladder)\b/.test(t)) return showLadder();
  if(/^(?:uji\s*cf|cf\s*uji)/.test(t)){
    if(!cur) return say('Belum ada pasaran aktif.');
    const entry = getCfEntry(cur);
    if(!entry || !entry.n) return say('Belum ada hasil Cloudflare AI tercatat untuk ' + cur + '. Ketik "tebak cf" dulu, lalu tunggu draw berikutnya masuk.');
    const labels = B.posLabels(L), chance = A.out * 10;
    const lines = labels.map(l => l + ': kena ' + (((entry.hits[l] || 0) / entry.n) * 100).toFixed(1) + '% dari ' + entry.n + ' uji (acak ' + chance + '%)');
    return say('UJI CLOUDFLARE AI ' + cur + ' (shadow-track, belum ikut campuran prediksi Ai):\n' + lines.join('\n') + (entry.pending ? '\n⏳ Ada tebakan menunggu hasil draw berikutnya.' : ''));
  }
  if(/^sumber/.test(t)){
...
function explainText(e){
  const top = e.top.map(t => t.name + ' ' + (t.w * 100).toFixed(0) + '%' + (t.fxPct != null ? ' [uji ala Formula X: ' + t.fxPct.toFixed(1) + '% dari ' + t.fxTotal + ' data]' : '')).join(', ');
...
// Ctrl (Kontrol N) & Tren N yang benar-benar dipakai pakar Formula X paling berbobot per posisi
// (sekarang ditentukan Ai sendiri, 10-60 — lihat CTRL_TREND_CANDIDATES/TREND_MIN/MAX di aiBrain.js).
// Kalau belum ada pakar fx yang cukup data/berbobot di top, TIDAK dikarang — dilaporkan apa adanya.
function fxAiRuleSummary(){
  if(typeof B === 'undefined' || !A.lastSt || !A.cur) return null;
  try{
    const ms = markets(); const mkt = ms[A.cur]; if(!mkt) return null;
    const L = A.lastSt.L, ctrls = [], trends = [];
    for(let p = 0; p < L; p++){
      const ex = B.explain(A.lastSt, p, mkt.C);
      const rows = (ex.top || []).concat(ex.best || []);
      const hit = rows.find(row => row.fxTrendN != null);
      if(hit){
        if(hit.fxCtrlN != null) ctrls.push(hit.fxCtrlN);
        trends.push(hit.fxTrendN);
      }
    }
    if(!trends.length) return null;
    const uniq = a => Array.from(new Set(a));
    return { controlN: ctrls.length ? uniq(ctrls).join('/') : 'otomatis', trendN: uniq(trends).join('/') };
  }catch(e){ return null; }
}
function lockGen1FromAi(pr){
  ...
  // v2: sebelumnya selalu '-' ("bukan dari Formula X, tidak relevan"). Sekarang Ai menentukan Kontrol N
  // & Tren N sendiri (10-60, per pakar Formula X paling berbobot di tiap posisi) — dilaporkan kalau ada,
  // 'otomatis'/'-' apa adanya kalau memang belum cukup data, tidak dikarang.
  const ruleInfo = fxAiRuleSummary();
  fxGen1LockedRule = ruleInfo
    ? { trendN: ruleInfo.trendN, controlN: ruleInfo.controlN, outN: '-', stats: 'Ai (Formula X, ctrl/tren 10-60 auto)' }
    : { trendN: '-', controlN: '-', outN: '-', stats: '-' };
...
function buildCard(){
  ...
  card.innerHTML =
    ...
    '<div class="row" style="gap:6px; flex-wrap:wrap; margin-bottom:10px;">' +
      ...
      '<button class="btn" data-cmd="audit">🛡️ Audit</button>' +
      '<button class="btn" data-cmd="tangga">🪜 Tangga</button>' +
...
function syncData(force){
  ...
  if(r){ try{ checkCfGuess(r.name, markets()); }catch(e){ console.error('Mode Ai: gagal cek tebakan Cloudflare AI', e); } }
  renderAll();
  if(r && A.states[r.name]){
    ...
    if(RM) auditActiveIfChanged(r.name); // Risk Manager: cek & cetak audit HANYA kalau statusnya benar-benar berubah
  }
}
...
function init(){
  ...
  load().then(() => {
    ...
    // Poin 4 blueprint: pemrosesan senyap tetap jalan utk pasaran LAIN yang sudah dimuat, walau layar
    // sedang membuka pasaran lain (tidak menunggu kartu Mode Ai dibuka, tidak memaksa render/pindah tampilan).
    if(RM) setInterval(guardSweepBackground, 30000);
    renderAll();
  });
}
