...
function renderJumlahSelisih(used){
  ...
  const N = parseInt(document.getElementById('jsRecoCount').value, 10) || 5;
  ...
  if(maxT < 1){
    ...
    for(let t = 0; t < maxT; t++){
      ...
      const jumlahOrdered = manualJumlah || bestCoverJsPart(subN, 'jumlahList', N);
      const selisihOrdered = manualSelisih || bestCoverJsPart(subN, 'selisihList', N);
  ...
  document.getElementById('simpulanJS').innerHTML =
    ...
    + `Window per baris: <b style="color:var(--ink);">${ROWS_SHOWN} data</b> · Jumlah digit rekomendasi: <b style="color:var(--ink);">${N}</b>.`;
...
document.getElementById('jsRecoCount').addEventListener('change', () => {
  if(lastJsUsed) renderJumlahSelisih(lastJsUsed);
});
...
function collectAllSettings(){
  return {
    ...
    js: {
      recoCount: document.getElementById('jsRecoCount').value
    },
    generator: {
...
function applyAllSettings(settings){
  ...
  if(js.recoCount != null) document.getElementById('jsRecoCount').value = js.recoCount;
...
// Sama seperti fxBuildGen2Pools, tapi aturan (selected/touched) diberikan lewat parameter —
// bukan diambil dari state FX_SELECTED/FX_TOUCHED yang sedang aktif. Dipakai untuk mengunci
// Gen 2 pertama kali (dengan konfigurasi Formula X yang SEDANG aktif di layar).
function fxBuildGen2PoolsWithRule(posLabels, used, selectedMap, touchedMap){
  if(!FX_FORMULAS_CACHE || !FX_RECOMMENDATIONS) return null;
  const byKey = {};
  FX_FORMULAS_CACHE.formulas.forEach(f => { byKey[f.key]=f; });
  const pools = [];
  for(let i=0; i<posLabels.length; i++){
    const recs = FX_RECOMMENDATIONS[posLabels[i]] || [];
    if(!recs.length){ pools.push([]); continue; }
    const label = posLabels[i];
    let chosenRec;
    if(touchedMap && touchedMap[label]){
      const selectedKey = selectedMap ? selectedMap[label] : null;
      chosenRec = recs.find(r => r.key === selectedKey) || recs[recs.length-1];
    } else {
      chosenRec = recs[recs.length-1];
    }
    const f = byKey[chosenRec.key];
    let pool = [];
    if(f){ try{ pool = f.fn(used)[i]||[]; }catch(e){} }
...
function fxBuildGen2Pools(posLabels, used){
  return fxBuildGen2PoolsWithRule(posLabels, used, FX_SELECTED, FX_TOUCHED);
}
...
// Gen 2 (2A/2B/2C) bisa dihitung ulang persis sesuai kombinasi Tren N/Control N/Out N/posisi ACKE
// miliknya sendiri masing-masing — walau slot lain atau layar utama sedang pakai kombinasi lain.
...
// Bangun pool satu slot Gen 2 dari SNAPSHOT lengkap (trendN, controlN, outN, selected, touched)
// yang tersimpan di s.rule — bukan dari konfigurasi Formula X yang sedang aktif di layar.
function fxBuildGen2PoolsFromSnapshot(posLabels, used, rule){
  if(!rule) return null;
  ...
  try{
    ...
    for(let i=0; i<posLabels.length; i++){
      ...
      let chosenRec;
      if(rule.touched && rule.touched[label]){
        const selKey = rule.selected ? rule.selected[label] : null;
        chosenRec = list.find(r => r.key === selKey) || list[list.length-1];
      } else {
        chosenRec = list[list.length-1];
      }
      const f = byKey[chosenRec.key];
      let pool = [];
      if(f){ try{ pool = f.fn(used)[i] || []; }catch(e){} }
...
function fxApplyGen2Rule(slot){
  ...
  const pools = fxBuildGen2PoolsFromSnapshot(lastPosLabels, lastHistoryNumbers, s.rule);
...
function rekapGen2Lines(posLabels, used){
  ...
  if(!anyLocked){
    const livePools = fxBuildGen2Pools(posLabels, used);
...
function renderGen2LockUI(slot){
  ...
  if(s.locked){
    ...
    // Tampilkan live preview Gen 2 (angka terburuk saat ini)
    if(lastPosLabels && lastHistoryNumbers.length && FX_RECOMMENDATIONS)
      renderDigitsRow('fxGen2'+slot+'DigitsRow', fxBuildGen2Pools(lastPosLabels, lastHistoryNumbers), lastPosLabels, 'rgba(217,112,122,.7)');
...
document.querySelectorAll('.fxGen2LockBtn').forEach(btn => {
  btn.addEventListener('click', ()=>{
    ...
    if(s.locked){
      ...
      const p = fxBuildGen2Pools(lastPosLabels, lastHistoryNumbers);
      if(!p){ alert('Formula X belum siap.'); return; }
      s.locked=true; s.pools=p; s.posLabels=lastPosLabels.slice(); s.fp=dataFingerprint(lastHistoryNumbers);
      // Simpan snapshot LENGKAP struktur Formula X saat ini — Tren N, Control N, Out N, dan
      // posisi ACKE yang dipilih (selected/touched) — bukan cuma posisinya. Supaya tiap slot
      // (2A/2B/2C) tetap independen: dikunci dengan kombinasi Formula X yang beda-beda pun,
      // masing-masing dihitung ulang dengan kombinasinya sendiri, bukan disamakan semua.
      s.rule = {
        trendN: document.getElementById('fxTrendN').value,
        controlN: document.getElementById('fxControlN').value,
        outN: document.getElementById('fxOutN').value,
...
function fxAutoGenerate(){
  ...
  if(withFilter){
    ...
    if(slotsToUse.length === 0){
      const livePools = fxBuildGen2Pools(lastPosLabels, lastHistoryNumbers);
...
function presetCollectGen2(){
  ...
  FX_GEN2_SLOTS.forEach(sl=>{
    ...
    data[sl] = (s.locked && s.rule) ? {
      ...
      selected: {...s.rule.selected}, touched: {...s.rule.touched}
    } : null;
  });
  return data;
}

// Terapkan aturan Gen2 dari preset. Kalau Data Historis sudah siap saat ini, langsung
// dihitung & dikunci; kalau belum, aturan tetap disimpan (s.rule) dan akan otomatis
// diterapkan begitu Data Historis selesai diproses (lewat fxAutoUnlockAll).
function presetApplyGen2(data){
  FX_GEN2_SLOTS.forEach(sl=>{
    ...
    if(entry){
      s.rule = {
        trendN: entry.trendN, controlN: entry.controlN, outN: entry.outN,
        selected: {...entry.selected}, touched: {...entry.touched}
      };
      const ok = fxApplyGen2Rule(sl);
      if(!ok){ s.locked=false; s.pools=null; s.posLabels=null; s.fp=''; }
    } else {
      s.rule=null; s.locked=false; s.pools=null; s.posLabels=null; s.fp='';
    }
    renderGen2LockUI(sl);
  });
}
