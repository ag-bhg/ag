...
document.getElementById('saveDataBtn').addEventListener('click', async () => {
  ...
  const newEntry = { name: trimmed, data, savedAt };
...
function firebaseLoadSelectedData({auto=false}={}){
  ...
  // Kunci juga tombol Mode (Normal/Semi/Auto) selama overlay ini tampil — bukan cuma
  // mengandalkan overlay menutupi klik, tapi eksplisit dikunci (lihat setModeButtonsDisabled
  // di automode.js), sejajar dengan pengunci yang sama di modeNormalBtn/semiAutoActivateBtn.
  if(typeof setModeButtonsDisabled === 'function') setModeButtonsDisabled(true);
  setTimeout(async () => {
    let _pipelineOk = true;
    try{
      ...
      // Mode Normal: ganti pasaran WAJIB balik ke SETTINGS_DEFAULTS (bukan pengaturan pasaran
      // yang tersimpan) — lihat resetAllSettings(). "Ingat pengaturan per pasaran" sengaja
      // tidak dipakai lagi di sini.
      const _mode = (typeof getAppMode === 'function') ? getAppMode() : 'normal';

      showLoadingOverlay('Menganalisis data...');
      ...
      // Muat ulang preset aktif setelah data & pengaturan siap — supaya pilihan preset
      // tetap terpasang setiap kali pasaran/data berganti, baik di Mode Auto/Semi maupun Normal.
      const _activePreset = (typeof getActivePresetName === 'function') ? getActivePresetName() : '';
      if(_activePreset && typeof presetLoad === 'function'){
        showLoadingOverlay('Menerapkan preset...');
        const _refreshCover = (_mode !== 'auto' && _mode !== 'semi');
        presetLoad(_activePreset, { refreshCoverData: _refreshCover });
      }

      // Tunggu pipeline async (Mode Auto/Semi) selesai sebelum lanjut validasi.
      // runAutoPipelineAfterFormulaX dipanggil dari dalam analyze() secara async —
      // kita tunggu lewat polling fxPipelineInFlight sampai selesai atau timeout 15 detik.
      if((_mode === 'auto' || _mode === 'semi') && typeof fxPipelineInFlight !== 'undefined'){
        showLoadingOverlay('Menjalankan pipeline otomatis...');
        await new Promise(resolve => {
          const start = Date.now();
          const check = setInterval(() => {
            if(!fxPipelineInFlight || Date.now() - start > 15000){
              clearInterval(check);
              resolve();
            }
          }, 100);
      ...
      _pipelineOk = errors.length === 0;
    } catch(e){
      _pipelineOk = false;
      console.error('firebaseLoadSelectedData error:', e);
    } finally {
      hideLoadingOverlay();
      if(typeof setModeButtonsDisabled === 'function') setModeButtonsDisabled(false);
      if(_pipelineOk){
        ...
        alert('Data termuat tapi ada proses yang belum selesai dengan benar.\n\nCoba muat ulang halaman lalu pilih pasaran kembali.\nJika masalah berlanjut, hubungi admin.');
      }
    }
  }, 30);
  return true;
...
function firebaseReceiveMasterData(value, auto=false){
  ...
  // Histori All Periode: setiap snapshot Firebase masuk (semua pasaran), catat kalau ada result baru.
  if(typeof onFirebaseMasterDataUpdate === 'function') onFirebaseMasterDataUpdate(firebaseMarketEntries);
}
